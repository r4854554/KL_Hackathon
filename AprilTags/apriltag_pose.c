...
double orthogonal_iteration(matd_t** v, matd_t** p, matd_t** t, matd_t** R, int n_points, int n_steps) {
    ...
    matd_t** p_res = (matd_t**)calloc(n_points, sizeof(matd_t*));
    for (int i = 0; i < n_points; i++) {
        p_res[i] = matd_op("M-M", p[i], p_mean);
    }

    // Compute M1_inv.
    matd_t** F = (matd_t**)calloc(n_points, sizeof(matd_t*));
    ...
    for (int i = 0; i < n_steps; i++) {
        ...
        *t = matd_multiply(M1_inv, M2);
        ...
        // Calculate rotation.
        matd_t** q = (matd_t**)calloc(n_points, sizeof(matd_t*));
...
void solve_poly_approx(double* p, int degree, double* roots, int* n_roots) {
    ...
    double* p_der = (double*)calloc(degree, sizeof(double));
    for (int i = 0; i < degree; i++) {
        p_der[i] = (i + 1) * p[i+1];
    }

    double* der_roots = (double*)calloc(degree - 1, sizeof(double));
    int n_der_roots;
...
matd_t* fix_pose_ambiguities(matd_t** v, matd_t** p, matd_t* t, matd_t* R, int n_points) {
	...
	double* rtData = (double*)calloc(9, sizeof(double));

	rtData[0] = MATD_EL(R_t_1, 0, 0);
	rtData[1] = MATD_EL(R_t_1, 0, 1);
	rtData[2] = MATD_EL(R_t_1, 0, 2);
	rtData[3] = MATD_EL(R_t_2, 0, 0);
	rtData[4] = MATD_EL(R_t_2, 0, 1);
	rtData[5] = MATD_EL(R_t_2, 0, 2);
	rtData[6] = MATD_EL(R_t_3, 0, 0);
	rtData[7] = MATD_EL(R_t_3, 0, 1);
	rtData[8] = MATD_EL(R_t_3, 0, 2);

    matd_t* R_t = matd_create_data(3, 3, rtData);
    matd_destroy(R_t_1);
    matd_destroy(R_t_2);
    matd_destroy(R_t_3);
	...
	double* rtzData = (double*)calloc(9, sizeof(double));

	rtzData[0] = r31 / hypotenuse;
	rtzData[1] = -r32 / hypotenuse;
	rtzData[2] = 0;
	rtzData[3] = r32 / hypotenuse;
	rtzData[4] = r31 / hypotenuse;
	rtzData[5] = 0;
	rtzData[6] = 0;
	rtzData[7] = 0;
	rtzData[8] = 1;

    matd_t* R_z = matd_create_data(3, 3, rtzData);
	...
	double* rGamma = (double*)calloc(9, sizeof(double));
    ...
    matd_t* R_gamma = matd_create_data(3, 3, rtzData);
    ...
    matd_t** v_trans = (matd_t**)calloc(n_points, sizeof(matd_t*));
    matd_t** p_trans = (matd_t**)calloc(n_points, sizeof(matd_t*));
    matd_t** F_trans = (matd_t**)calloc(n_points, sizeof(matd_t*));
	...
	double* m1Data = (double*)calloc(9, sizeof(double));
	double* m2Data = (double*)calloc(9, sizeof(double));
    ...
    matd_t* M1 = matd_create_data(3, 3, m1Data);
    matd_t* M2 = matd_create_data(3, 3, m2Data);
    ...
    double roots[4];
    int n_roots;
	double* pData = (double*)calloc(5, sizeof(double));
    ...
    solve_poly_approx(pData, 4, roots, &n_roots);
...
void estimate_tag_pose_orthogonal_iteration(
        ...
        int nIters) {
    double scale = info->tagsize/2.0;
	double* scale1Data = (double*)calloc(3, sizeof(double));
	double* scale2Data = (double*)calloc(3, sizeof(double));
	double* scale3Data = (double*)calloc(3, sizeof(double));
	double* scale4Data = (double*)calloc(3, sizeof(double));
    ...
    matd_t* p[4] = {
        matd_create_data(3, 1, scale1Data),
        matd_create_data(3, 1, scale2Data),
        matd_create_data(3, 1, scale3Data),
        matd_create_data(3, 1, scale4Data)};
    matd_t* v[4];
    for (int i = 0; i < 4; i++) {
		double* vData = (double*)calloc(3, sizeof(double));

		vData[0] = (info->det->p[i][0] - info->cx) / info->fx;
		vData[1] = (info->det->p[i][1] - info->cy) / info->fy;
		vData[2] = 1;
        v[i] = matd_create_data(3, 1, vData);
    }
