namespace AprilTagsSharp
{
    ...
    {
        TagFamily tagFamily; //tag family
        ...
        public AprilTag(string threshold = "canny", bool debug = false, string familyname = "tag36h11", double sigma = 0.8, int nthread = 1, int minarea = 400)
        {
            ...
            {
                case "tag36h10":
                    this.tagFamily = new Tag36h10();
                    break;
                case "tag36h11":
                    this.tagFamily = new Tag36h11();
                    break;
                case "tag25h9":
                    this.tagFamily = new Tag25h9();
                    break;
                case "tag16h5":
                    this.tagFamily = new Tag16h5();
                    break;
                default:
                    this.tagFamily = new Tag36h11();
                    break;
        ...
        public ArrayList Detect(int height, int width, byte[] data)
        {
            var src = new Mat(height, width, MatType.CV_8UC4, data);

            return detect(src);
        }

        public ArrayList Detect(byte[] data)
        {
            var mat = Cv2.ImDecode(data, ImreadModes.Unchanged);
